...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=MeleApp \
	-X github.com/cosmos/cosmos-sdk/version.ServerName=meled \
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/meled-manager
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/meled

build: go.sum clean
	go build -mod=mod $(BUILD_FLAGS) -o build/meled-manager ./cmd/meled-manager
	go build -mod=mod $(BUILD_FLAGS) -o build/meled ./cmd/meled
...
localnet-start: build-linux localnet-stop
	@if ! [ -f build/node0/meled/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/meled:Z mele/core testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test --chain-id test ; fi
