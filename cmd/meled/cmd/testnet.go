...
func testnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a Mele testnet",
...
Example:
	meled testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "meled", "Home directory of the node's daemon configuration")
...
func InitTestnet(
	...
	addresses := []string{
		"mele17zc58s96rxj79jtqqsnzt3wtx3tern6ad2rn0h", // Validator
		"mele1dfjns5lk748pzrd79z4zp9k22mrchm2a5t2f6u", // Test 1
		"mele1c7nn5mt43m37t0zmqwh6rslrgcr3gd4pxqutpj", // Test 2
		"mele1e2k26e6yz3kwysp896mwpf5u5r0nh8zx0yvtsz", // Delegator
	}

	accStakingTokens := sdk.NewInt(100000000000000)
	coins := sdk.Coins{
		sdk.NewCoin(config.DefaultStableDenomination, accStakingTokens),
		sdk.NewCoin(config.DefaultDenomination, accStakingTokens),
	}
	...
	for i := 0; i < numValidators; i++ {
		...
		valTokens := sdk.TokensFromConsensusPower(100, sdk.DefaultPowerReduction) // cosmos-sdk 0.44.0 updates

		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(config.DefaultStableDenomination, valTokens),
	...
	for _, address := range addresses {
		...
		genBalances = append(genBalances, banktypes.Balance{Address: accAddress.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, authtypes.NewBaseAccount(accAddress, nil, 0, 0))
	}
...
func initGenFiles(
	...
	stakingGenState.Params.BondDenom = config.DefaultStableDenomination
