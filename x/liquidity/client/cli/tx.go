...
import (
	...
	"github.com/melechain/mele/x/liquidity/types"
)
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Get deposit coins
			depositCoins, err := customutils.ParseCoinsNormalized(args[1])
			if err != nil {
				return err
			}
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			poolCoin, err := customutils.ParseCoinNormalized(args[1])
			if err != nil {
				return err
			}
...
The only supported swap-type is 1. For the detailed swap algorithm, see https://github.com/melechain/mele
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Get offer coin
			offerCoin, err := customutils.ParseCoinNormalized(args[2])
			if err != nil {
				return err
			}
